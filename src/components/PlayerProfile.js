  ...
  return (
    <div className="player-profile">
      ...
      <div className="profile-header">
        ...
        <div className="player-details">
          ...
          <div className="player-badges">
            <span className={`performance-badge ${performanceRating.color}`}>
              {performanceRating.icon} {performanceRating.rating}
            </span>
            <span className={`speed-badge ${speedRating.color}`}>
              {speedRating.icon} {speedRating.rating}
            </span>
            {getStreakBadges(stats.bestStreak).map((badge, index) => (
              <span key={index} className="streak-badge">{badge}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="profile-stats-grid">
        ...
        <div className="stat-card">
          <div className="stat-icon">⚡</div>
          <div className="stat-number">{formatTime(stats.avgResponseTime)}</div>
        ...
        <div className="profile-section">
          ...
          <div className="performance-chart">
            <div className="chart-item">
              <div className="chart-label">Accuracy Rate</div>
              <div className="chart-bar-container">
                <div 
                  className="chart-bar accuracy-bar"
                  style={{ width: `${stats.accuracy}%` }}
                ></div>
                <span className="chart-value">{stats.accuracy.toFixed(1)}%</span>
              </div>
            </div>
            <div className="chart-item">
              <div className="chart-label">Speed Efficiency</div>
              <div className="chart-bar-container">
                <div 
                  className="chart-bar speed-bar"
                  style={{ width: `${Math.max(10, 100 - (stats.avgResponseTime / 30) * 100)}%` }}
                ></div>
                <span className="chart-value">{formatTime(stats.avgResponseTime)}</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="profile-section">
            ...
            <div className="question-breakdown">
              {Object.entries(questionStats)
                ...
                .map(([questionNum, qStats]) => (
                  <div key={questionNum} className="question-stat">
                    <div className="question-header">
                      <span className="question-label">Question {questionNum}</span>
                      <span className={`question-accuracy ${qStats.correct > 0 ? 'correct' : 'incorrect'}`}>
                        {qStats.correct}/{qStats.total} ({((qStats.correct / qStats.total) * 100).toFixed(0)}%)
                      </span>
                    </div>
                    <div className="question-details">
                      <span className="question-time">Avg: {formatTime(qStats.avgTime)}</span>
                    </div>
                  </div>
                ))}
            </div>
